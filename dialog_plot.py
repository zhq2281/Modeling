...
from ui_dialog_plot import Ui_Dialog
...
class MainDialogImgBW(QDialog,Ui_Dialog):
    def __init__(self):
        ...
        self.exec_()
    def valchange(self):
        self.label.setNum(self.horizontalSlider.value())
        self.F.fig.suptitle(str(self.horizontalSlider.value()))
        print(str(self.horizontalSlider.value()))
        self.plotcos()


        # self.label.setFont("微软雅黑",self.horizontalSlider.value())
    def plotcos(self):
        t = np.arange(0.0, 5.0, 0.01)
        s = np.cos((2+self.horizontalSlider.value()) * np.pi * t)
        self.F.axes.cla()
